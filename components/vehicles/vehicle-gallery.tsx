...
export function VehicleGallery({ images, vehicleTitle }: VehicleGalleryProps) {
  ...
  return (
    <div className="space-y-3">
      {/* Main Image Display */}
      <div className="relative aspect-[16/10] w-full rounded-2xl overflow-hidden bg-slate-950 border border-slate-200 group shadow-md">
        <Image
          src={currentImage}
        ...
        <button
          ...
          className="absolute top-4 right-4 bg-slate-950/75 hover:bg-amber-500 hover:text-slate-950 text-white text-xs font-semibold px-3 py-1.5 rounded-lg backdrop-blur-md flex items-center gap-1.5 transition-all shadow-md"
        >
      ...
      {lightboxOpen && (
        <div className="fixed inset-0 z-50 bg-slate-950/95 backdrop-blur-xl flex flex-col items-center justify-center p-4">
          <button
            ...
            className="absolute top-6 right-6 text-white hover:text-amber-400 p-2 rounded-full bg-slate-900 border border-slate-700"
          >
            <X className="w-7 h-7" />
          </button>
