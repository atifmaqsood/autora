...
const partsStats = [
  { value: 10000, suffix: "+", label: "Parts & Accessories Exported" },
...
export default function PartsAccessoriesPage() {
  ...
  const categories = getAllPartsCategories();
          ...
          {categories.map((card, index) => {
            ...
            return (
              <Reveal key={card.title} delay={index * 85} className={card.layout}>
                <Link
                  href={`/parts-accessories/${card.slug}`}
                  className="group relative flex h-full overflow-hidden rounded-[18px] border border-[#315671] bg-[#14314B] p-6 shadow-lg transition-all duration-500 hover:-translate-y-1 hover:border-[#F97316]/75 hover:shadow-[0_22px_50px_rgba(0,0,0,0.24)]"
                >
                  <Image
                    src={card.image}
                    alt={card.title}
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#0B1F33]/80 via-[#0B1F33]/35 to-transparent" />
                  <div className="absolute inset-x-6 top-6 flex items-center justify-between">
                    <span className="rounded-full border border-white/20 bg-[#0B1F33]/70 px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.16em] text-white backdrop-blur">
                      Explore Products ({card.products.length})
                    </span>
                  ...
                  <div className="relative z-10 mt-auto max-w-[390px]">
                    ...
                    <h3 className="text-[21px] font-black leading-[1.05] text-white md:text-[24px]">
                      {card.title}
                    </h3>
                    <p className="mt-2.5 text-[12px] font-bold leading-[1.45] text-slate-200">
                      {card.subtitle}
                    </p>
                  </div>
                </Link>
      ...
      <section className="mx-auto grid max-w-7xl grid-cols-1 gap-12 px-4 pt-28 sm:px-6 lg:grid-cols-12 lg:px-8">
        ...
        <RevealStagger staggerDelay={85} className="space-y-4 lg:col-span-7">
          {processSteps.map((step) => (
            <div
              key={step.title}
              className="grid grid-cols-[auto_1fr] gap-5 rounded-3xl border border-slate-800 bg-[#102941] p-6 shadow-xl hover:border-[#F97316] transition-colors"
            >
          ...
          <div className="relative overflow-hidden rounded-[32px] border border-slate-800 bg-[#0B1F33] p-10 md:p-16 lg:p-20 text-center shadow-2xl space-y-6">
            <div className="absolute inset-0 z-0">
              ...
              <div
                ...
                style={{
                  background:
                    "linear-gradient(to top, var(--agtp-primary, #0B1F33) 0%, color-mix(in srgb, var(--agtp-primary, #0B1F33) 80%, transparent) 50%, transparent 100%)"
                }}
              />
            </div>
